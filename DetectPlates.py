...
#extract ra plate
def extract(img):
    img_gray, threshold = Preprocess.preprocess(img)
    kernel_length = 3
    ...
    img_verticle = cv2.dilate(opening, verticle_kernel)
    img_hori = cv2.dilate(opening, hori_kernel)

    alpha = 0.5
    beta = 1.0 - alpha

    img_addweight = cv2.addWeighted(img_verticle, alpha, img_hori, beta, 0.0)

    img_bin = 255 - img_addweight
    img_final = cv2.erode(img_bin, kernel)

    imgContours, contours, npaHierarchy = cv2.findContours(img_final, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
    idx = 0
    listofpalte = []
    for c in contours:
        ...
        # crop and save
        #if float(w/h) < 5 :
        if (70 < w < 130 and 10 < h < 35) and float(w / h) < 5:
            new_img = img[y+2:y + h-2, x:x + w]
            listofpalte.append(new_img)
    plates = []
    for plate in listofpalte:
        _, plate = Preprocess.correct_skew(plate)
        plates.append(plate)
        cv2.imwrite("plate" + str(idx) + ".png", plate)
        idx += 1
    return plates[0]
