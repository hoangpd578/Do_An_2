...
def checkIfPossibleChar(contour):
    x, y, w, h = cv2.boundingRect(contour)
    if (w*h > MIN_PIXEL_AREA and w > MIN_PIXEL_WIDTH and h > MIN_PIXEL_HEIGHT and
            MIN_ASPECT_RATIO < float(w/h) and float(w/h) < MAX_ASPECT_RATIO):
...
def extract_char(img):
    ...
    listOfPossibleChars = []
    imgContours, contours, npaHierarchy = cv2.findContours(thresh, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)
